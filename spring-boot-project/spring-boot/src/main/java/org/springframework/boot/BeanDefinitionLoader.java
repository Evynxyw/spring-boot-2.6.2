...
class BeanDefinitionLoader {
	...
	BeanDefinitionLoader(BeanDefinitionRegistry registry, Object... sources) {
		// 检查 registry 参数是否为空，如果为空则抛出 IllegalArgumentException 异常
		Assert.notNull(registry, "Registry must not be null");
		// 检查 sources 参数是否为空，如果为空则抛出 IllegalArgumentException 异常
		Assert.notEmpty(sources, "Sources must not be empty");

		// 将 sources 参数赋值给实例变量 this.sources
		this.sources = sources;

		// 创建注解读取器实例，用于处理注解配置的 bean，如: @Configuration、@Bean、@Controller 等
		this.annotatedReader = new AnnotatedBeanDefinitionReader(registry);

		// 创建 XML 读取器实例，用于处理 XML 文件里配置的 bean
		this.xmlReader = (XML_ENABLED ? new XmlBeanDefinitionReader(registry) : null);

		// 如果存在 Groovy 配置，则创建 GroovyBeanDefinitionReader 实例，用于处理 Groovy 配置的 bean
		this.groovyReader = (isGroovyPresent() ? new GroovyBeanDefinitionReader(registry) : null);

		// 创建 ClassPathBeanDefinitionScanner 实例，用于扫描类路径中的 bean 定义
		this.scanner = new ClassPathBeanDefinitionScanner(registry);

		// 向扫描器添加排除过滤器，以排除指定的 sources
	 ...
	 */
	void load() {
		// 遍历所有源进行加载，此处仅加载：class smoketest.tomcat.SampleTomcatApplication
